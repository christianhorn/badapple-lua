...
function drawframedouble(frame)
    	local counter
	counter = frame * 10800
    	local r
    	for y = bmpheight, 1, -1 do
			...
			r = string.byte(content,counter)
			counter = counter + 1
            		img:set(x*2,   y*2+70, _clr.set(-1, r, r, r) )
            		img:set(x*2,   y*2-1+70, _clr.set(-1, r, r, r) )
            		img:set(x*2-1, y*2+70, _clr.set(-1, r, r, r) )
            		img:set(x*2-1, y*2-1+70, _clr.set(-1, r, r, r) )
        	end
    	end
end
...
for i=0, 102, 1 do
	local fullpic = "/bamedia/tiny_120x90-macropic/pic-" .. string.format("%07d", i)
	readimage(fullpic); 

	for frame=0, 7, 1 do

		-- drawframe(frame);
		drawframedouble(frame);
		...
		draw_char(fps/10, 10, 10);
	        ...
	        _lcd:update();
		rb.sleep(5) -- sleeps 1/100 sec, needed to keep sound playing
	end
