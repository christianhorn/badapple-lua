 ...
 * 
 * media prepare: 
 * Takes one frame (i.e. frame 1), skips the next one, and
 * uses then the next one (frame 3) for comparison.
 * Writes diffs into an output file.  
 * One output-file per diff.  If the 2 frames
 * are identical, the output file is 0.
 *
 * Effectively, that skips then one frame on the output
 * device.
 */
...
int main(int argc, char *argv[]) {
	...
	char diffbasepath[512]="../bamedia/tiny_240x180_differ_half/image-";
	...
	if ( argc != 2 ) {
		...
		printf("I will compare image-00000<number>.bmp and image-00000<number+2>.bmp .\n");
		return 1;
	}
	picnumber = atoi( argv[1] );
	sprintf(pic1path,"%s%07d.bmp", basepath, picnumber);
	sprintf(pic2path,"%s%07d.bmp", basepath, picnumber + 2);
